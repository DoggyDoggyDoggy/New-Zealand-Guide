...
class NewZealandHistoryRepositoryImplTest {
    private val repository = NewZealandHistoryRepositoryImpl()

    @Test
    fun `getNewZealandHistory should return all chapters with correct titles and paragraphs`() {
        ...
        val result = repository.getNewZealandHistory()

        assertThat(result.paragraphsTitles)
            .containsExactlyElementsIn(expectedTitles)
            .inOrder()

        assertThat(result.paragraphs)
            .containsExactlyElementsIn(expectedParagraphs)
            .inOrder()
    }
}